...
function getnamabagian($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT nama FROM t_kode_bagian WHERE id = '$id'")->row();
	return $q_ambil->nama;
}
...
function getdisposisi($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT t_disposisi.kpd_yth, t_surat_masuk.id from t_disposisi left join t_surat_masuk ON t_disposisi.id_surat = t_surat_masuk.id ")->row();
	$data	= empty($q_ambil) ? "-" : $q_ambil->kpd_yth;
	return $data;
}
function getlevel($id) {
	$CI =& get_instance();
	$q_ambil	= $CI->db->query("SELECT level FROM t_level WHERE id = '$id'")->row();
	return $q_ambil->level;
}
